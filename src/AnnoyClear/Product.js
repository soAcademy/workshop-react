const product = ({ data }) => {
  return (
    <>
      <div className="w-full bg-gray-700 m-4 p-4 rounded-md">
        <div>
          <img className="w-full rounded-md" src={data.image} />
        </div>
        <div>
          <h1 className="text-xl text-white mt-4 font-bold">{data.name}</h1>
        </div>
        <p className="text-gray-300 mt-2">{data.description}</p>
        <div className="text-right text-amber-500 text-xs">{data.price}</div>
        <div>
...
// อันเก่าที่ต้องเติมแบบ Hardsell
// const product = () => {
//   return (
//     <>
//       <div className="w-full bg-gray-700 m-4 p-4 rounded-md">
